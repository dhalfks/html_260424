...
function extractValue(strValue){
    ...
    // search regEx pattern을 이용한 검색 후 index 리턴
    let reg = /[\+\-\*\/]/;
    console.log(strValue.search(reg));
    let firstNumber = strValue.substring(0,strValue.search(reg));
    let lastNumber = strValue.substring(strValue.search(reg)+1);
    let op = strValue.charAt(strValue.search(reg));
